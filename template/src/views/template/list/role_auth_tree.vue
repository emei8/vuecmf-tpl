        ...
        >
            ...
            <template v-else>
                <template v-for="(item,index) in menu_operate">
                    <div :key="index" class="menu_item"><i-checkbox  :indeterminate="indeterminate[index]"  v-model="menuCheckAll[index]" @click.prevent.native="checkAll(index)">{{ item.title }}</i-checkbox></div>
                    <i-checkbox-group :key="index"  v-model="operateList[index]" @on-change="checkAllGroupChange(operateList[index],index,item.operate.length)"  v-if="item.operate.length > 0" >
                        <i-row>
                            <i-col span="6" v-for="(item2,index2) in item.operate" :key="index2"><i-checkbox  :label="item2.id">{{ item2.label }}</i-checkbox></i-col>
                        </i-row>
    ...
    methods: {
        checkAll: function(index){
            ...
            if (that.menuCheckAll[index]) {
                ...
                that.menu_operate[index].operate.forEach(function (item) {
                    operateItem.push(item.id)
                })
                that.$set(that.operateList,index,operateItem)
            } else {
        ...
        saveOperate(){
            ...
            that.operateList.forEach(function (item,index) {
                if(item != null) operateList[index] = item
            })

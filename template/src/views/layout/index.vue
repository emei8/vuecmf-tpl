<template>
    <i-layout>
        ...
        <i-layout>
            <i-sider hide-trigger  collapsible  :collapsed-width="50" v-model="sideCollapse" >
                <div class="collapse-menu" @click.prevent="collapse" >
                    <i-icon type="ios-log-in" v-show="!sideCollapse" title="折叠" />
                    <i-icon type="ios-log-out" v-show="sideCollapse" title="展开" />
                </div>
                <vc-side-menu :side-collapse="sideCollapse" ref="vc_side_menu"></vc-side-menu>
            </i-sider>
...
export default {
    ...
    methods:{
        ...
        restSideMenuSelect(){
            //DOM渲染后的回调
            this.$nextTick(() => {
                this.$refs.vc_side_menu.$refs.sideMenu.updateOpened()
                this.$refs.vc_side_menu.$refs.sideMenu.updateActiveName()
            })
        }
